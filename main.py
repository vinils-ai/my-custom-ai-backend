...
# Safe nltk setup on server startup
try:
...
# 1. Deep Neural Network Architecture (Must match Colab architecture)
...
# 2. Load trained model file safely
...
def predict_response(request: ChatRequest):
    try:
        ...
        if prob.item() > 0.70:
            for intent in intents['intents']:
                if tag == intent['tag']:
                    ...
                    return {"reply": reply, "tag": tag, "confidence": float(prob.item())}
        ...
        return {"reply": "Internal server processing error.", "error": str(e)}